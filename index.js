...
app.post("/orders", verifyToken, async (req, res) => {
  try {
    ...
    const productObjectIds = data.cartData.map((item) => new ObjectId( item.productId));
...
const products = await productsCollection.find({ _id: { $in: productObjectIds } }).toArray();
    ...
    //     buyingPrice: dbPrice, // Securely mapped database price
    //     quantity: quantity,
    //   });
    // }

    // // 4. Construct the final payload safely
    // const orderPayload = {
    //   ...data,
    ...
    // // 5. Insert the document using the native driver
